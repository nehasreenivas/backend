...
const PostRequest = async (_url, payload) => {
    ...
    return await axios.post(url, isFormData ? payload : JSON.stringify(payload), {
...
export const loginUser = async (notifier, userDetails, dispatch, callback) => {
    const resp = await PostRequest('/login_user', userDetails);

    if (!resp.status) {
        if (resp.msg) notifier(resp.msg);
        if (callback) callback(null);  // failed login
        return;
    }
    ...
    if (callback) callback(resp.response);  // success
};
