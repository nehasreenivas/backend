    ...
    useEffect(() => {
        ...
        const fetchInterviewDetails = async () => {
            try {
                ...
                notifier('Failed to load interviews');
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className='page-wrapper'>
            ...
            <div className='page-container'>
                <span style={{ color: 'var(--ui-color)', fontSize: '1.4rem', fontWeight: 'bold', padding: '10px' }}>
                    Interviews</span>
                {loading && <div style={{ padding: '20px', textAlign: 'center' }}>Loading interviews...</div>}
                {error && <div style={{ padding: '20px', textAlign: 'center', color: 'red' }}>{error}</div>}
                {!loading && interviewList.length === 0 && (
                    <div style={{ padding: '20px', textAlign: 'center', color: '#666' }}>
                        No interviews scheduled yet. Schedule interviews from the Posted Jobs page.
                    </div>
                )}
                <div className='interview-list-container'>
                    {
                        !loading && interviewList.map((item, index) => {
