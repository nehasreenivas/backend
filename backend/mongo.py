...
def initialize_database(db):
    ...
    if not db.usercred.find_one({'username': 'Tymor'}):
        db.usercred.insert_one({'username': 'tymor', 'password': 'tym2@123', 'name': 'TYMOR', 'interviewer': True})
    ...
    if not db.usercred.find_one({'username': 'neha_kotapalli'}):
        db.usercred.insert_one({'username': 'neha_kotapalli','password': 'neha@123','name': 'Neha Kotapalli','interviewer': False})
        
    if not db.usercred.find_one({'username': 'student'}):
        db.usercred.insert_one({'username': 'student','password': 'student@123','name': 'Student','interviewer': False})
...
After that, I started working at Illinois Software Company as a junior developer. A year into that job, I was promoted to senior developer, which meant I handled my own projects and checked the junior developers’ before they went to the client. I held that position until the company went under last month.''',
    ...
    if not db.user_details.find_one({'username': 'neha_kotapalli'}):
        db.user_details.insert_one({
            'username': 'neha_kotapalli',
            'profile_image': 'https://example.com/profile.jpg',
            'first_name': 'Neha',
            'last_name': 'Kotapalli',
            'e_mail': 'neha.kotapalli@example.com',
            'phone': '9966324422',
            'job': 'Example',
            'about': 'I am an AI developer with experience in Python and machine learning.',
            'experience': [['Intern (2024 - Present)', 'XYZ Company']],
            'current_salary': '800000',
            'expected_salary': '1000000',
            'resume_location': ''
        })
        
    if not db.user_details.find_one({'username': 'student'}):
        db.user_details.insert_one({
            'username': 'student',
            'profile_image': 'https://example.com/student.jpg',
            'first_name': 'Student',
            'last_name': 'User',
            'e_mail': 'student@example.com',
            'phone': '9999999999',
            'job': 'Student',
            'about': 'I am a student learning software development and AI.',
            'experience': [['Intern (2025 - Present)', 'Learning Company']],
            'current_salary': '0',
