...
import json
from analyze_answer import process_all_answers
...
@app.route('/interview_questions/<job_role>', methods=['GET'])
@cross_origin(supports_credentials=True)
def get_interview_questions(job_role):
    question_bank_lower = {k.lower(): v for k, v in interview_questions.question_bank.items()}
    questions = question_bank_lower.get(job_role.lower(), [])
...
def analyze_answer():
    try:
        ...
        audio_file_path = data.get("audio_file_path")  # <-- frontend should send this

        if not all([candidate_id, interview_id, question_index, question, audio_file_path]):
            return jsonify({"status": "error", "message": "Missing required parameters"}), 400

        # ✅ Call your analyze_answer.py logic
        result = process_answer(candidate_id, interview_id, question_index, question, audio_file_path)
...
@app.route('/process_full_interview', methods=['POST'])
@cross_origin(supports_credentials=True)
def process_full_interview():
    data = request.json
    candidate_id = data.get("candidate_id")
    interview_id = data.get("interview_id")
    ...
    result = process_all_answers(candidate_id, interview_id, questions, audio_file_path)
    return jsonify(result)

@app.route('/analizeCandidateGaze', methods=['GET'])
@cross_origin(supports_credentials=True)
def analize_candidate_gaze():
    file_path = request.args.get('filePath', None)
    if not file_path:
        return {'msg': 'File Path not found'}, 400

    # Run gaze analysis
    result = analyze_gaze(file_path)
    return jsonify(result)
